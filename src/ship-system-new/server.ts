...
// Bohai Sea / Tangshan Coast Land Mask
const SF_LAND_MASK = [
  // Hebei Coast (Tangshan area)
  [
    { lat: 39.10, lng: 118.20 },
    { lat: 39.10, lng: 118.80 },
    { lat: 38.80, lng: 119.00 },
    { lat: 38.50, lng: 118.80 },
    { lat: 38.50, lng: 118.20 }
  ].map(p => toLocal(p.lat, p.lng)),
  // Tianjin Coast
  [
    { lat: 39.00, lng: 117.70 },
    { lat: 39.10, lng: 118.00 },
    { lat: 39.00, lng: 118.20 },
    { lat: 38.80, lng: 117.80 },
    { lat: 38.80, lng: 117.60 }
  ].map(p => toLocal(p.lat, p.lng)),
  // Liaoning Coast (Yingkou/Dandong area)
  [
    { lat: 40.60, lng: 122.10 },
    { lat: 40.80, lng: 122.50 },
    { lat: 40.00, lng: 122.80 },
    { lat: 39.90, lng: 122.00 }
  ].map(p => toLocal(p.lat, p.lng)),
  // Shandong Peninsula
  [
    { lat: 37.50, lng: 122.20 },
    { lat: 37.80, lng: 122.80 },
    { lat: 37.00, lng: 122.50 },
    { lat: 36.80, lng: 122.00 }
  ].map(p => toLocal(p.lat, p.lng)),
  // Caofidian Port specifically (人工岛/port area)
  [
    { lat: 38.95, lng: 118.45 },
    { lat: 38.97, lng: 118.55 },
    { lat: 38.88, lng: 118.60 },
    { lat: 38.86, lng: 118.48 }
  ].map(p => toLocal(p.lat, p.lng))
];
...
function initGlobalLand() {
  ...
  try {
    ...
    function processPolygon(coords: any[]) {
      ...
      // Add to landPolygons for pathfinding if in Bohai Sea / Tangshan area
      // Area: Bohai Sea region around 38-40N, 117-120E
      if (maxLng > 117 && minLng < 120 && maxLat > 37.5 && minLat < 40.5) {
...
class ShipSimulation {
  ...
  T3 = 5.0;
  TE = 0.5;
  // 15.9 knots = 8.18 m/s (fixed initial speed)
  U = 8.18;

  // Start in Bohai Sea, south of Caofidian port
  x = 5000;
  y = -3000;
  psi = 0;
  r = 0;
  ...
  update(dt: number) {
    ...
    if (distToLand < 300) { // Warning distance at 300m
      this.collision_warning = true;
    }
    
    // Hard collision prevention - increased threshold to 150m to prevent passing through land
    if (distToLand < 150) {
      // Calculate normal vector from land using gradient
      const eps = 20;
      ...
      if (nLen > 0.01) {
        // Push away from land - stronger push
        const pushStrength = Math.max(15, (150 - distToLand) * 0.6);
        this.x += (nx / nLen) * pushStrength;
        this.y += (ny / nLen) * pushStrength;
      } else {
        // Fallback: simple push back against velocity
        this.x -= this.U * Math.cos(this.psi) * dt * 2;
        this.y -= this.U * Math.sin(this.psi) * dt * 2;
      }
      
      // Significantly reduce speed on collision
      this.U *= 0.3;
      this.r *= 0.5;
    }
    
    // Additional check: if somehow inside land (distance = 0), emergency push out
    if (distToLand === 0) {
      const eps = 50;
      let bestDir = { x: 0, y: 0 };
      let bestDist = 0;
      for (let angle = 0; angle < 360; angle += 15) {
        ...
        const testX = this.x + Math.cos(rad) * eps;
        const testY = this.y + Math.sin(rad) * eps;
        const testDist = getDistanceToPolygons(testX, testY, this.obstacles);
        if (testDist > bestDist) {
          bestDist = testDist;
          bestDir = { x: Math.cos(rad), y: Math.sin(rad) };
        }
      }
      this.x += bestDir.x * 100;
      this.y += bestDir.y * 100;
      this.U *= 0.2;
    }
...
app.get('/api/state', (req, res) => {
  ...
  // Calculate distance to target and ETA
  let distanceToTarget = 0;
  let etaSeconds = 0;
  if (ship.currentWaypointIndex < ship.waypoints.length) {
    const target = ship.waypoints[ship.currentWaypointIndex];
    distanceToTarget = Math.hypot(target.x - ship.x, target.y - ship.y);
    etaSeconds = ship.U > 0 ? distanceToTarget / ship.U : 0;
  } else if (ship.waypoints.length > 0) {
    // All waypoints reached, use last waypoint for reference
    const lastTarget = ship.waypoints[ship.waypoints.length - 1];
    distanceToTarget = Math.hypot(lastTarget.x - ship.x, lastTarget.y - ship.y);
    etaSeconds = ship.U > 0 ? distanceToTarget / ship.U : 0;
  }
  ...
  res.json({
    ...
    predicted: predicted,
    targetX: ship.waypoints.length > 0 ? ship.waypoints[ship.currentWaypointIndex]?.x : null,
    targetY: ship.waypoints.length > 0 ? ship.waypoints[ship.currentWaypointIndex]?.y : null,
    distanceToTarget,
    etaSeconds,
    params: {
      K: ship.K,
...
app.post('/api/reset', (req, res) => {
  // Reset to initial position in Bohai Sea
  ship.x = 5000;
  ship.y = -3000;
  ship.psi = 0;
  ship.r = 0;
