...
import { Activity, Compass, Wind, Gauge, AlertTriangle, MapPin, Cpu, Clock, Route } from 'lucide-react';
import { toGeo } from './Map';

export default function Telemetry({ 
  x, y, psi, r, delta, delta_c, U, collision_warning, ddpg_active, distanceToTarget, etaSeconds
}: { 
  x: number, y: number, psi: number, r: number, delta: number, 
  delta_c: number, U: number, collision_warning: boolean, ddpg_active: boolean,
  distanceToTarget?: number, etaSeconds?: number
}) {
  ...
  const formatDistance = (meters: number) => {
    if (!meters || meters <= 0 || !isFinite(meters)) return '--';
        ...
        <div className="grid grid-cols-2 gap-x-8 gap-y-6">
          ...
          <div className="col-span-2 grid grid-cols-2 gap-4 mt-1">
            <div className="bg-white/5 rounded-2xl p-3 border border-white/10 shadow-inner">
              <div className="text-[9px] text-slate-300 font-black uppercase mb-1 tracking-widest flex items-center gap-1">
                <Route className="w-3 h-3 text-cyan-400"/>距目标距离
              </div>
              <div className="text-base font-mono text-cyan-400 font-black">{formatDistance(distanceToTarget)}</div>
            </div>
            <div className="bg-white/5 rounded-2xl p-3 border border-white/10 shadow-inner">
              <div className="text-[9px] text-slate-300 font-black uppercase mb-1 tracking-widest flex items-center gap-1">
                <Clock className="w-3 h-3 text-amber-400"/>预计到达
              </div>
              <div className="text-base font-mono text-amber-400 font-black">{formatETA(etaSeconds)}</div>
            </div>
          </div>
        </div>
      </div>
